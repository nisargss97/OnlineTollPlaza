...
#define ADMINPASS "qwerty123"						//PASSKEY TO REGISTER ADMIN
#define ff fflush(stdin);							
#define err printf("\n\nflag\n\n");

typedef struct{ 									//Structure for Transactions
	double amt;
	char time_stamp[40];
	char id[21];
	int month;
...
typedef struct{ 									//Structure for Users
	char id[21];
	char pwd[21];
	char name[30];
	char email[30];
...
typedef struct{  									//Structure for Admins
	char id[21];
	char pwd[21];
	double income;
...
typedef struct{ 									//Temporary Structure 
	user u;
	admin a;
	int flag;
} stemp;
...
//Function Containing Main User Menu
...
//Function Containing Menu for logged in user (Profile)
...
//					printf("\nCar no : %s \n",u.id);
 			...
 			case 3:
 					...
 					FILE* g=fopen("transaction.txt","ab+");
 					trans t;
 					time_t ltime=time(NULL);
 					struct tm tm=*localtime(&ltime);
 					strcpy(t.time_stamp,asctime(&tm));
...
//Function for Admin Registration PASSKEY: "qwerty123"
...
// Admin Operations Menu for logged in Admin
...
// System Transaction Function, User inputs Car no.
...
// Function to display transactions of a particular user u
...
// Function to display transactions for a particular day to admin a
...
void admin_transaction(admin a)
{
	...
	FILE* f=fopen("transaction.txt","ab+");
	printf("\nEnter date for which transactions are to be printed : \n");
	printf("\nEnter year (>1900) : ");
	ff;scanf("%d",&y);
	printf("\nEnter month (1-12) : ");
	ff;scanf("%d",&m);
...
//		fread(&v,sizeof(v),1,f);
		...
		if(d==v.day&&y==v.year&&m==v.month)
		{	
			printf("\n%d . User : %s",i,v.id);
			if(v.type)
			{
				printf("\n\nType : Toll Transaction \nDate : %sAmount : %lf\n\n",v.time_stamp,v.amt);
			}
			else
			{
				printf("\n\nType : Credit \nDate : %sAmount : %lf\n\n",v.time_stamp,v.amt);
...
// Function to display admin a's income for a particular day
...
void admin_income(admin a)
{
	...
	system("clear");ff;
	int d,m,y;
	printf("\nADMIN's PROFILE : \n\nEnter date foor which Total Income is to be printed :  ");
	printf("\nEnter date for which transactions are to be printed : \n");
	printf("\nEnter year (>1900) : ");
	ff;scanf("%d",&y);
	printf("\nEnter month (1-12) : ");
	...
	printf("\nTotal Income for required day : %lf\n",a.income);
	printf("\nPress Enter to continue. 	");
	fclose(g);
	fclose(f);	
	ff;
